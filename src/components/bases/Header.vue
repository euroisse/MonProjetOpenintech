  ...
  <div class="relative h-max w-screen overflow-x-hidden">
    ...
    >
      <div class="container flex items-center justify-between">
        ...
        >
          <ul
            class="flex flex-col items-center gap-8 lg:flex-row text-[#6B7280]"
          >
              ...
              <RouterLink
                ...
                :class="{
                  'bg-white shadow-lg border-b-2 border-black text-black hover:text-black':
                    route.path === link.to,
                  'bg-white border-none hover:text-black':
                    route.path !== link.to,
                }"
        ...
        <div
          class="flex flex-row items-center gap-2 flex-wrap md:hidden lg:flex"
        >
...
const logout = async () => {
  try {
    await axios.post(
      "https://booking.openintech.app/api/logout",
      {},
      {
        headers: {
...
const showHeader = computed(
  () => !["/login", "/registerForm"].includes(route.path)
);
